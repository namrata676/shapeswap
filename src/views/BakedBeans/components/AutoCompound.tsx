...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersNodeContract }) => {
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      farmersNodeContract.methods.getMyEggs(account).call().then( function( info ) {
          setTotalNodes(info)
         })
       } 
  ...
  return (
    <StyledCard>
      <RowBlockBetween>      
        <StyledImg src='./images/bakedbeans.png' alt='status_logo' />
        <div className="d-flex justify-content-between">
          <Label labelSize='20px'>{totalNodes} beans</Label>           
        </div>        
        <Link to={{ pathname: "https://bakedbeans.io/?ref=0x10bA6854F7C9A003c5898D5f8fafb2bBd3A8966C" }} target="_blank">
          <Label1 labelSize="8px"> baked beans dapp </Label1>
        </Link>
      </RowBlockBetween>
