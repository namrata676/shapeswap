...
const CreateNewNode = ({account, setRefetch, refetch, emeraldNodeContract, emeraldTokenContract, token}) => {
  

  const [nodePrice, setNodePrice] = useState(0)
  const [nodeName, setNodeName] = useState('');
  const [emeraldDecimals, setEmeraldDecimals] = useState('');

  useEffect(()=>{

  if(token==="emerald_nodes"){
   emeraldTokenContract.methods.decimals().call().then( function( info ) {

      setEmeraldDecimals(info)
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const amount1 = (10*(10**(parseInt(emeraldDecimals)))).toLocaleString('fullwide', { useGrouping: false });
        console.log("amount@@",amount1)
        const nftTxn = await emeraldTokenContract.methods.createNodeWithTokens(nodeName,amount1).send({from: account});
        if(nftTxn){
