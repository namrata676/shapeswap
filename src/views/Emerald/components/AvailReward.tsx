...
const AvailReward = ({account,setRewards, token, emeraldNodeContract, emeraldTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             emeraldNodeContract.methods.viewTotalReward(account).call().then( function( info ) {
              
                 setRewardAvailable(((info?.RewardToBeClaimed)/expo).toFixed(4))
                 setRewards(((info?.RewardToBeClaimed)/expo).toFixed(4))
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="emerald_nodes"){
        const nftTxn = await emeraldNodeContract.methods.claimRewards().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $EM</Label>          
        </div>
