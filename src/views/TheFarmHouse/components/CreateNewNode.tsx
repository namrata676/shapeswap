...
const CreateNewNode = ({account, setRefetch, refetch, farmersNodeContract,token}) => {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await farmersNodeContract.methods.plantSeeds(referral).send({from: account, value: amount1})
        if(nftTxn){
          alert("Seeds Planted Successfully !")
          setRefetch(!refetch)
        }else{
          alert("Error in Planting Seeds !")
        }
        }
    }
      else {
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={createNewNodeFunc}>buy seeds</StyledButton>       
      </StyledCardBody>
    </StyledCard>
  )
}
