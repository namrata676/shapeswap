...
const AvailReward = ({account,setRewards, token, gorillaNodeContract, gorillaTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             if(token==="gorilla")
             gorillaNodeContract.methods.getAllNodesRewards(account).call().then( function( info ) {
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await gorillaTokenContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $BANANA</Label>          
        </div>
