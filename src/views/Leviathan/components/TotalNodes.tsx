...
const TotalNodes = ({ title, val1, val2, img, account, refetch, nebulaTokenContract, nebulaNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="nebula_nodes"){
        nebulaNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
