...
const CreateNewNode = ({account, setRefetch, refetch, lavaNodeContract, lavaTokenContract, token}) => {
  ...
  if(token==="gorilla"){
   lavaTokenContract.methods.decimals().call().then( function( info ) {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await lavaTokenContract.methods.createNodeWithTokens(nodeName,amount1).send({from: account});
        if(nftTxn){
