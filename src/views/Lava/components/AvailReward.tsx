...
const AvailReward = ({account,setRewards, token, lavaNodeContract, lavaTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             if(token==="gorilla")
             lavaNodeContract.methods.getAllNodesRewards(account).call().then( function( info ) {
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await lavaTokenContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $LAVA</Label>          
        </div>
