...
const AvailReward = ({account,setRewards, token, farmersNodeContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             farmersNodeContract.methods.rubyRewards(account).call().then( function( info ) {
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await farmersNodeContract.methods.sellRubies().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await farmersNodeContract.methods.harvestRubies(referral).send({from: account});
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={compoundReward}>Rehire</StyledButton>
      </StyledCardBody>
    </StyledCard>
  )
}
