...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersNodeContract }) => {
  ...
  return (
    <StyledCard>
      <RowBlockBetween>        
        <StyledImg src='./images/rubyminesmall.png' alt='status_logo' />
        <div className="d-flex justify-content-between">
          <Label labelSize='20px'>{totalNodes} Miners</Label>          
        </div>
        <Link to={{ pathname: "https://www.rubymine.money/" }} target="_blank">
          <Label1 labelSize="8px"> RubyMine Dapp </Label1>
        </Link>
      </RowBlockBetween>
    </StyledCard>
  )
