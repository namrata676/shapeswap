...
const TotalNodes = ({ title, val1, val2, img, account, refetch, goldenTokenContract, goldenNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="golden_society"){
        goldenNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
