...
const AvailReward = ({account,setRewards, token, goldenNodeContract, goldenTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             goldenNodeContract.methods.availableRewardsFor(account).call().then( function( info ) {
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await goldenTokenContract.methods.claim().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $GDSY</Label>          
        </div>
