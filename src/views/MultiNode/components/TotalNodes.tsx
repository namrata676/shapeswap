...
const TotalNodes = ({ title, val1, val2, img, account, refetch, MnodeTokenContract, MnodeNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="Mnode_nodes"){
        MnodeNodeContract.methods.getTotalNodes().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
