...
const TotalNodes = ({ title, val1, val2, img, account, refetch, MnodeNodeContract }) => {
  const [totalNodes, setTotalNodes] = useState('0');
  const [MNodes, setMNodes] = useState('0');
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      MnodeNodeContract.methods.getUserNodeCount(account).call().then( function( info ) {
          setTotalNodes(info)
         })
      MnodeNodeContract.methods.getUserNodeCount(account).call().then( function( info1 ) {
          setMNodes(info1)
         })
    ...
    <StyledCard>
      <RowBlockBetween>
        <StyledCardBody>
          ...
          {/* <RowBlock>
            <Label size='18px'>M Nodes {MNodes}</Label>            
          </RowBlock> */}
        </StyledCardBody>
        {/* <StyledCardBody> */}
