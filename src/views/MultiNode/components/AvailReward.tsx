...
const AvailReward = ({account,setRewards, token, MnodeNodeContract, MnodeTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             MnodeNodeContract.methods.getCurrentInterest(account).call().then( function( info ) {
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="Mnode_nodes"){
        const nftTxn = await MnodeNodeContract.methods.withdrawInterest().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} Mnode</Label>          
        </div>
