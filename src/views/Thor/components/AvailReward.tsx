...
const AvailReward = ({account,setRewards, token, thorNodeContract, thorTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             thorNodeContract.methods._getRewardAmountOf(account).call().then( function( info ) {
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await thorTokenContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $THOR</Label>          
        </div>
