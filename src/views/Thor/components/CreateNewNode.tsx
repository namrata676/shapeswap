...
const CreateNewNode = ({account, setRefetch, refetch, thorNodeContract, thorTokenContract, token}) => {
  ...
  useEffect(()=>{
   ...
   thorTokenContract.methods.decimals().call().then( function( info ) {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await thorTokenContract.methods.createNodeWithTokens(nodeName).send({from: account});
        if(nftTxn){
