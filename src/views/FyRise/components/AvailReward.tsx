...
const AvailReward = ({account,setRewards,setReward,setTax,token, farmersNodeContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             farmersNodeContract.methods.blockRewards(account).call().then( function( info ) {
            //     setTax(((info/expo)*.14).toFixed(4)) 
            //     setReward(((info/expo) + setTax).toFixed(4))
                 setRewardAvailable((info/expo).toFixed(6))
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await farmersNodeContract.methods.sellLands().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await farmersNodeContract.methods.buildLands(referral).send({from: account});
        if(nftTxn){
