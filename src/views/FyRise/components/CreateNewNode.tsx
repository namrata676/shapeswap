...
const CreateNewNode = ({account, setRefetch, refetch, farmersNodeContract,token}) => {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await farmersNodeContract.methods.buyBlocks(referral).send({from: account, value: amount1})
        if(nftTxn){
          alert("Farmers Hired Successfully !")
          setRefetch(!refetch)
        }else{
          alert("Error in Hiring Farmers !")
        }
        }
    }
      else {
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={createNewNodeFunc}>Buy Blocks</StyledButton>        
      </StyledCardBody>
    </StyledCard>
  )
}
