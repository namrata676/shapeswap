...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersNodeContract }) => {
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      farmersNodeContract.methods.getMyMiners(account).call().then( function( info ) {
        ...
        //  farmersNodeContract.methods.getMyBlocks(account).call().then( function( info ) {
        //   setTotalBlocks(info)
        //  })  
       } 
  ...
  return (
    <StyledCard>
      <RowBlockBetween>      
        <StyledImg src='./images/flyrise.png' alt='status_logo' />
        <div className="d-flex justify-content-between">
          {/* <Label labelSize='20px'>{totalBlocks} Blocks</Label> */}
          <Label labelSize='20px'>{totalNodes} Builders</Label>           
        </div>        
        <Link to={{ pathname: "https://fyrise.io/?ref=0x10bA6854F7C9A003c5898D5f8fafb2bBd3A8966C" }} target="_blank">
          <Label1 labelSize="8px"> FyRise Dapp </Label1>
        </Link>
      </RowBlockBetween>
