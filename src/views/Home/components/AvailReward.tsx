...
const AvailReward = ({account,nodeContract, setRewards, compoundContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             compoundContract.methods.getAllNodesRewards(account).call().then( function( info ) {
                 setRewardAvailable((info/expo).toFixed(2))
                 setRewards((info)/expo)
                })
              } 
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await  compoundContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $SMRT</Label>          
        </div>
