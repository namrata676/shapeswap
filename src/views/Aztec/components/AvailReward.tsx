...
const AvailReward = ({account,setRewards, token, projectxNodeContract, projectxTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             projectxNodeContract.methods._getRewardAmountOf(account).call().then( function( info ) {
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="projectx_nodes"){
        const nftTxn = await projectxTokenContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $PXT2</Label>          
        </div>
