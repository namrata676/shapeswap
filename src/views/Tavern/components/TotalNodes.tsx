...
const TotalNodes = ({ title, val1, val2, img, account, refetch, projectxTokenContract, projectxNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="projectx_nodes"){
        projectxNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
