...
const CreateNewNode = ({account, setRefetch, refetch, farmersNodeContract,token}) => {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await farmersNodeContract.methods.buyDiamonds(referral).send({from: account, value: amount1})
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={createNewNodeFunc}>Buy Diamonds</StyledButton>
      </StyledCardBody>
    </StyledCard>
  )
}
