...
const AvailReward = ({account,setRewards, token, farmersNodeContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             farmersNodeContract.methods.diamondRewards(account).call().then( function( info ) {
                 setRewardAvailable((info/expo).toFixed(5))
                 setRewards((info/expo).toFixed(5))
                })
               }}
              }
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await farmersNodeContract.methods.sellDiamonds().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await farmersNodeContract.methods.harvestDiamonds(referral).send({from: account});
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={claimReward}>Sell Diamonds</StyledButton>           
        <StyledButton onClick={compoundReward}>Harvest</StyledButton>
      </StyledCardBody>
    </StyledCard>
  )
}
