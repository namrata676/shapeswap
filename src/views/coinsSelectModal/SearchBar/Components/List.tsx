    ...
    return (
        <ul className="listCoin">
            {tokenList.map((item) => (
                ...
                <img src={item?.icon} width='13.95px'height="15.94px" alt='weth' />
                <button className="networkButton" type="button"  onClick={()=> { onSelect(item) }} style={{ marginBottom: '5%', marginLeft: '1%', border: 'none', color: 'black', backgroundColor: 'white'}}>{item?.symbol}</button>
              </div></li>
            ))}
        </ul>
