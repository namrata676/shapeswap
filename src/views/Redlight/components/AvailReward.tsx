...
const AvailReward = ({account,setRewards, token, redlightNodeContract, redlightNodeHelperContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             if(token==="redlight_nodes")
             await redlightNodeContract.methods.getAddressRewards(account).call().then().then( async function( info ) {
                 setRewardAvailable((info/expo).toFixed(4))
                 setRewards((info/expo).toFixed(4))
                 
          

             await redlightNodeContract.methods.getMansionsOf(account).call().then( async function( info2 ) {
              setNodeIds(info2)
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="redlight_nodes"){
        const nftTxn = await redlightNodeHelperContract.methods.claimAll(NodeIds).send({from: account});
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        <Heading scale="sm">
          Available Distric Rewards
        </Heading>
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $PLAYMATES</Label>          
        </div>
