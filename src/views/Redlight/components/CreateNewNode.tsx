...
const CreateNewNode = ({account, setRefetch, refetch, redlightNodeHelperContract, redlightTokenContract, token}) => {
  

  const [nodePrice, setNodePrice] = useState(0)
  const [nodeName, setNodeName] = useState('');
  const [redlightDecimals, setredlightDecimals] = useState('');

  useEffect(()=>{

  if(token==="redlight_nodes"){
   redlightTokenContract.methods.decimals().call().then( function( info ) {

      setredlightDecimals(info)
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        if(token==="redlight_nodes"){
        const nftTxn = await redlightNodeHelperContract.methods.createNodeWithTokens(nodeName).send({from: account});
        if(nftTxn){
