...
const TotalNodes = ({ title, val1, val2, img, account, refetch, redlightTokenContract, redlightNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="redlight_nodes"){
        redlightNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
