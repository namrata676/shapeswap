...
const AvailReward = ({account,setRewards, token, farmersNodeContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             farmersNodeContract.methods.getUserUnpaidEarnings(account).call().then( function( info ) {
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await farmersNodeContract.methods.claimPendingRewards().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await farmersNodeContract.methods.compoundDividends(referral).send({from: account});
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        ...
        <StyledButton onClick={claimReward}>Collect Rewards</StyledButton>           
        <StyledButton onClick={compoundReward}>Compound 0% Tax</StyledButton>
      </StyledCardBody>
    </StyledCard>
  )
}
