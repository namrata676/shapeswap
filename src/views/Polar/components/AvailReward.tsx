...
const AvailReward = ({account,setRewards, token, polarNodeContract, polarTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             polarNodeContract.methods.getClaimableRewardsOf(account).call().then( function( info ) {
                 setRewardAvailable((info[0]/expo).toFixed(4))
                 setRewards((info[0]/expo).toFixed(4))
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="polar_nodes"){
          const nftTxn = await polarNodeContract.methods.claimRewardsAll("0x6C1c0319d8dDcb0ffE1a68C5b3829Fd361587DB4",account).send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} $POLAR</Label>          
        </div>
