...
const CreateNewNode = ({account, setRefetch, refetch, polarNodeContract, polarTokenContract, token}) => {
  ...
  if(token==="polar_nodes"){
   polarTokenContract.methods.decimals().call().then( function( info ) {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await polarTokenContract.methods.createNodeWithTokens(nodeName).send({from: account});
        if(nftTxn){
