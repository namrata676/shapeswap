...
const CreateNewNode = ({account, setRefetch, refetch, nebulaNodeContract, nebulaTokenContract, token}) => {
  ...
  if(token==="nebula_nodes"){
   nebulaTokenContract.methods.decimals().call().then( function( info ) {

      setNebulaDecimals(info)
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        if(token==="nebula_nodes"){
        const amount1 = (10*(10**(parseInt(nebulaDecimals)))).toLocaleString('fullwide', { useGrouping: false });
        console.log("amount@@",amount1)
        const nftTxn = await nebulaTokenContract.methods.createNodeWithTokens(nodeName,amount1).send({from: account});
        if(nftTxn){
