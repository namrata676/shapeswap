...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersTokenContract }) => {
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      farmersTokenContract.methods.balanceOf(account).call().then( function( info ) {
  ...
  return (
    <StyledCard>
      <RowBlockBetween>      
        <StyledImg src='./images/alphashares.png' alt='status_logo' />
        <div className="d-flex justify-content-between">
          <Label labelSize='20px'>{totalNodes} Alpha Shares</Label>          
        </div>        
        <Link to={{ pathname: "https://www.alphanodes.finance/dashboard" }} target="_blank">
          <Label1 labelSize="8px"> Alpha Shares </Label1>
        </Link>
      </RowBlockBetween>
