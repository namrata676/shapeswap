...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersNodeContract }) => {
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      farmersNodeContract.methods.getCirculatingSupply().call().then( function( info ) {
          setTotalNodes((info/expo).toFixed(2))
         })
         farmersNodeContract.methods.totalSupply().call().then( function( info ) {
          setTotalNodes1((info/expo).toFixed(2))
         })
       } 
    ...
    <StyledCard>
      <RowBlockBetween>
        <StyledCardBody>
          ...
          <RowBlock> 
            <Label size='10px'>{totalNodes} sphere</Label>            
          </RowBlock>
          <Heading scale="sm" mb="8px">
            total supply 
          </Heading>
          <RowBlock> 
            <Label size='10px'>{totalNodes1} sphere</Label>            
          </RowBlock>
        </StyledCardBody>
      </RowBlockBetween>
    </StyledCard>
