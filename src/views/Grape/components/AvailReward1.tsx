...
const AvailReward1 = ({account,setRewards, setRewards1, token, GrapeFinanceGrapeContract, GrapeFinanceWineContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
            ...
            GrapeFinanceWineContract.methods.getTotalRewards(account).call().then( function( info ) {
                 setRewardAvailable((info/expo).toFixed(4))
                 setRewards1((info/expo).toFixed(4))
                })
               }}
              }
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        const nftTxn = await GrapeFinanceWineContract.methods.claim().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        ...
        const nftTxn = await GrapeFinanceWineContract.methods.compound().send({from: account}); 
        if(nftTxn){
  ...
  return (
    <StyledCard>
      <StyledCardBody>
        <Heading scale="sm">
          wine available rewards
        </Heading>
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='20px'>{availReward} wine</Label>          
        </div>
