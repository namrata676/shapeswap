...
const TotalNodes = ({ title, val1, val2, img, account, refetch, farmersNodeContract }) => {
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      farmersNodeContract.methods.hatcheryMiners(account).call().then( function( info ) {
          setTotalNodes(info)
         })
       } 
  ...
  return (
    <StyledCard>
      <RowBlockBetween>      
        <StyledImg src='./images/RoastBeefMatic.png' alt='status_logo' />
        <div className="d-flex justify-content-between">
          <Label labelSize='20px'>{totalNodes} Beef</Label>           
        </div>        
        <Link to={{ pathname: "https://roastbeefmatic.io/#/?ref=0x10bA6854F7C9A003c5898D5f8fafb2bBd3A8966C" }} target="_blank">
          <Label1 labelSize="8px"> RoastBeef Matic Dapp </Label1>
        </Link>
      </RowBlockBetween>
