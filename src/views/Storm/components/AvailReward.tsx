...
const AvailReward = ({account,setRewards, token, stormNodeContract, stormTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             stormNodeContract.methods.getAllNodesRewards(account).call().then( function( info ) {
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="storm_nodes"){
        const nftTxn = await stormTokenContract.methods.cashoutAll().send({from: account});
        if(nftTxn){
  ...
  const compoundReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="storm_nodes"){
        const nftTxn = await stormTokenContract.methods.compoundAllNodes().send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='16px'>{availReward} $STORMZV2</Label>          
        </div>
