...
const TotalNodes = ({ title, val1, val2, img, account, refetch, stormTokenContract, stormNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="storm_nodes"){
        stormNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
