...
const TotalNodes = ({ title, val1, val2, img, account, refetch, cryptoNodeContract }) => {
  const [totalNodes, setTotalNodes] = useState('Nan');
  const [totalNodes1, setTotalNodes1] = useState('Nan');
  ...
  useEffect(() => {
    ...
    if(account !== undefined){
      ...
      cryptoNodeContract.methods.checkNodesLvl1(account).call().then( function( info ) {
          setTotalNodes(info)
         })
      cryptoNodeContract.methods.checkNodesLvl2(account).call().then( function( info2 ) {
    ...
    <StyledCard>
      <RowBlockBetween>
        <StyledCardBody>
          ...
          <RowBlock>
            <Label size='8px'>{totalNodes}</Label>            
          </RowBlock>
          <Heading scale="sm" mb="4px">
          Big Nodes
          </Heading>
          <RowBlock>
            <Label size='8px'>{totalNodes1}</Label>            
          </RowBlock>
        </StyledCardBody>
        {/* <StyledCardBody> */}
