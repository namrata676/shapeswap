...
const AvailReward = ({account,setRewards, token, cryptoNodeContract, cryptoTokenContract}) => {
   ...
   const { ethereum } = window;
         ...
         if (ethereum) {
           if(account !== undefined){
             ...
             cryptoNodeContract.methods.checkNodesMoney(account).call().then( function( info ) {
                 setRewardAvailable((info/expo).toFixed(3))
                 setRewards((info/expo).toFixed(3))
 ...
 useEffect(()=>{
   ...
   const claimReward = async() => {
    ...
    if (ethereum) {
      if(account !== undefined){
        if(token==="crypto_nodes"){
        const nftTxn = await cryptoNodeContract.methods.claimNodesMoney(account).send({from: account});
        if(nftTxn){
    ...
    <StyledCard>
      <StyledCardBody>
        ...
        <div className="d-flex justify-content-between" style={{width:'190px'}}>
          <Label labelSize='26px'>{availReward} $CRND</Label>          
        </div>
