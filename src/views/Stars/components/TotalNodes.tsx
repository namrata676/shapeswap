...
const TotalNodes = ({ title, val1, val2, img, account, refetch, starTokenContract, starNodeContract,token }) => {
  ...
  const getTotalNodes = () => {
    ...
    if (ethereum) {
     ...
     if(token==="star_nodes"){
        starNodeContract.methods.totalNodesCreated().call().then( function( info ) {
             setTotalNodes(info)
           })
         } 
        }
