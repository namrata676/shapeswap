...
const CreateNewNode = ({account, setRefetch, refetch, starNodeContract, starTokenContract, token}) => {
  ...
  if(token==="star_nodes"){
   starTokenContract.methods.decimals().call().then( function( info ) {
  ...
  const createNewNodeFunc = async () => {
      ...
      if (ethereum) {
        ...
        const nftTxn = await starTokenContract.methods.createNodeWithTokens(nodeName,amount1).send({from: account});
        if(nftTxn){
